...
function patch$1(fs2) {
  ...
  function patchLchmod(fs22) {
    ...
    fs22.lchmodSync = function(path2, mode) {
      var fd = fs22.openSync(path2, constants$5.O_WRONLY | constants$5.O_SYMLINK, mode);
      var threw = true;
      var ret;
      try {
  ...
  function patchLutimes(fs22) {
    if (constants$5.hasOwnProperty("O_SYMLINK") && fs22.futimes) {
      fs22.lutimes = function(path2, at, mt, cb) {
        fs22.open(path2, constants$5.O_SYMLINK, function(er, fd) {
          if (er) {
            if (cb) cb(er);
            return;
          }
      ...
      fs22.lutimesSync = function(path2, at, mt) {
        var fd = fs22.openSync(path2, constants$5.O_SYMLINK);
        var ret;
        var threw = true;
        try {
...
utils$9.checkPath = function checkPath(pth) {
...
const fs$k = fs$m;
const { checkPath: checkPath2 } = utils$9;
...
async function copy$2(src, dest, opts = {}) {
  ...
  const { srcStat, destStat } = await stat$7.checkPaths(src, dest, "copy", opts);
  await stat$7.checkParentPaths(src, srcStat, dest, "copy");
...
async function onDir$1(srcStat, destStat, src, dest, opts) {
  ...
  await asyncIteratorConcurrentProcess(await fs$g.opendir(src), async (item) => {
    const srcItem = path$c.join(src, item.name);
    const destItem = path$c.join(dest, item.name);
    const include = await runFilter(srcItem, destItem, opts);
    if (include) {
      const { destStat: destStat2 } = await stat$7.checkPaths(srcItem, destItem, "copy", opts);
...
function copySync$1(src, dest, opts) {
  ...
  const { srcStat, destStat } = stat$6.checkPathsSync(src, dest, "copy", opts);
  stat$6.checkParentPathsSync(src, srcStat, dest, "copy");
...
function copyDirItem(item, src, dest, opts) {
  ...
  const { destStat } = stat$6.checkPathsSync(srcItem, destItem, "copy", opts);
...
const { areIdentical: areIdentical$1 } = stat$8;
...
var utils$8 = { stringify: stringify$8, stripBom: stripBom$1 };
let _fs;
try {
  _fs = gracefulFs;
} catch (_) {
...
const { stringify: stringify$7, stripBom } = utils$8;
...
async function outputFile$1(file2, data, encoding = "utf-8") {
  ...
  return fs$7.writeFile(file2, data, encoding);
}
...
const { stringify: stringify$6 } = utils$8;
...
const { stringify: stringify$5 } = utils$8;
...
async function move$1(src, dest, opts = {}) {
  ...
  const { srcStat, isChangingCase = false } = await stat$5.checkPaths(src, dest, "move", opts);
  await stat$5.checkParentPaths(src, srcStat, dest, "move");
...
function moveSync(src, dest, opts) {
  opts = opts || {};
  const overwrite = opts.overwrite || opts.clobber || false;
  const { srcStat, isChangingCase = false } = stat$4.checkPathsSync(src, dest, "move", opts);
  stat$4.checkParentPathsSync(src, srcStat, dest, "move");
...
const fs$4 = /* @__PURE__ */ getDefaultExportFromCjs(lib);
var chokidar = {};
var utils$7 = {};
...
const setFsWatchListener = (path2, fullPath, options, handlers) => {
  ...
  if (cont) {
    ...
    if (!watcher2) return;
    watcher2.on(EV_ERROR$2, async (error) => {
...
const createWindow = async () => {
  win = new electron.BrowserWindow({
    ...
    webPreferences: {
      preload: path$f.join(__dirname, "preload.cjs"),
  ...
  if (!electron.app.isPackaged) {
    ...
    win.loadFile(path$f.join(__dirname, "../index.html"));
  }
...
async function listDirectory(dirPath) {
  const names = await fs$4.readdir(dirPath);
  const entries = [];
  for (const name of names) {
    const full = path$f.join(dirPath, name);
    try {
      ...
      entries.push({
        ...
        ext: path$f.extname(name).toLowerCase()
      });
    } catch (e) {
    }
  }
...
function startWatcher(dirPath) {
  ...
  try {
    ...
    watcher.on("add", (p) => win == null ? void 0 : win.webContents.send("fs:changed", { type: "add", path: p })).on("addDir", (p) => win == null ? void 0 : win.webContents.send("fs:changed", { type: "addDir", path: p })).on("unlink", (p) => win == null ? void 0 : win.webContents.send("fs:changed", { type: "unlink", path: p })).on("unlinkDir", (p) => win == null ? void 0 : win.webContents.send("fs:changed", { type: "unlinkDir", path: p })).on("change", (p) => win == null ? void 0 : win.webContents.send("fs:changed", { type: "change", path: p })).on("error", (error) => {
      try {
        if (error.code === "EBUSY") {
...
electron.ipcMain.handle("fs:chooseDir", async () => {
  const res = await electron.dialog.showOpenDialog(win, { properties: ["openDirectory", "createDirectory"] });
  if (res.canceled || res.filePaths.length === 0) return null;
  return res.filePaths[0];
});
electron.ipcMain.handle("fs:list", async (_e, dirPath) => {
  return await listDirectory(dirPath);
});
electron.ipcMain.handle("fs:setWatch", async (_e, dirPath) => {
  startWatcher(dirPath);
  return true;
});
electron.ipcMain.handle("fs:openPath", async (_e, targetPath) => {
  await electron.shell.openPath(targetPath);
  return true;
});
electron.ipcMain.handle("fs:trash", async (_e, targetPath) => {
  if (electron.shell.trashItem) {
    await electron.shell.trashItem(targetPath);
  } else {
    await fs$4.remove(targetPath);
  }
  return true;
});
electron.ipcMain.handle("fs:rename", async (_e, fromPath, toName) => {
  const toPath = path$f.join(path$f.dirname(fromPath), toName);
  await fs$4.rename(fromPath, toPath);
  return toPath;
});
electron.ipcMain.handle("fs:newFolder", async (_e, parent, baseName = "New Folder") => {
  let i = 0;
  let candidate = path$f.join(parent, baseName);
  while (await fs$4.pathExists(candidate)) {
    i += 1;
    candidate = path$f.join(parent, `${baseName} (${i})`);
  }
  await fs$4.mkdir(candidate);
  return candidate;
});
function isWindows() {
...
function knownFoldersWindows() {
  const user = process.env.USERPROFILE || os.homedir();
  return [
    { name: "Desktop", path: path$f.join(user, "Desktop"), icon: "desktop" },
    { name: "Documents", path: path$f.join(user, "Documents"), icon: "documents" },
    { name: "Downloads", path: path$f.join(user, "Downloads"), icon: "downloads" },
    { name: "Pictures", path: path$f.join(user, "Pictures"), icon: "pictures" },
    { name: "Music", path: path$f.join(user, "Music"), icon: "music" },
    { name: "Videos", path: path$f.join(user, "Videos"), icon: "videos" }
  ];
}
electron.ipcMain.handle("fs:getDrives", async () => {
  if (isWindows()) return await listDrivesWindows();
  return [{ name: "Root", path: "/", type: "drive" }];
});
electron.ipcMain.handle("fs:getKnownFolders", async () => {
  if (isWindows()) return knownFoldersWindows();
  const home = os.homedir();
  return [
    { name: "Home", path: home, icon: "home" },
    { name: "Desktop", path: path$f.join(home, "Desktop"), icon: "desktop" },
    { name: "Documents", path: path$f.join(home, "Documents"), icon: "documents" },
    { name: "Downloads", path: path$f.join(home, "Downloads"), icon: "downloads" },
    { name: "Pictures", path: path$f.join(home, "Pictures"), icon: "pictures" }
  ];
});
electron.ipcMain.handle("fs:getFreeSpace", async (_e, anyPath) => {
  try {
    if (!anyPath) return null;
    if (process.platform === "win32") {
      const m = anyPath.match(/^([A-Za-z]:)\\?/);
      const driveLetter = m ? m[1].charAt(0) : null;
      if (driveLetter) {
        const tryCmd = async (cmd) => {
          try {
            const out = await new Promise((resolve, reject) => {
              require$$0$5.exec(cmd, { windowsHide: true }, (err, stdout, stderr) => {
                if (err) return reject(err);
        ...
        try {
          ...
          const out3 = await new Promise((resolve, reject) => {
            require$$0$5.exec(cmd3, { windowsHide: true }, (err, stdout, stderr) => {
              if (err) return reject(err);
              resolve((stdout || stderr).toString());
            });
          });
      ...
      const out = await new Promise((resolve, reject) => {
        require$$0$5.exec(cmd, (err, stdout, stderr) => {
          if (err) return reject(err);
...
electron.ipcMain.handle("fs:getFileIcon", async (_e, targetPath, sizeHint = "normal") => {
  try {
    const ext = path$f.extname(targetPath).toLowerCase();
    const { getThumbnail } = await Promise.resolve().then(() => require("./thumbnail-CZSIh_FA.js"));
    ...
    const img = await electron.app.getFileIcon(targetPath, opts);
    const dataUrl = img.toDataURL();
    return dataUrl;
  } catch (err) {
    try {
      const img2 = electron.nativeImage.createFromPath(targetPath);
...
electron.ipcMain.handle("fs:revealInFolder", async (_e, targetPath) => {
  try {
    electron.shell.showItemInFolder(targetPath);
    return true;
  } catch {
    await electron.shell.openPath(path$f.dirname(targetPath));
    return true;
  }
});
